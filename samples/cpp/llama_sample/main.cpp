...
int tmain(int argc, tchar* argv[]) {
    try {
        ...
        if (argc != 6) {
            slog::info << "Usage : " << argv[0] << " <path_to_model> <path_to_vocab_file> <prompt_text> <max_sequence_length> <device_name>" << slog::endl;
	    return EXIT_FAILURE;
        }
	...
	const std::string device_name = TSTRING2STRING(argv[5]);
        // const int set_ppp = strtol(argv[5], NULL, 10);
        auto startTime = Time::now();
        llama_tokenizer llama_tokenizer(vocab_file);
        std::vector<int> prompt = llama_tokenizer.tokenizer(prompt_str);
        // std::vector<int> prompt = {5618, 338, 26163, 29973};
	    ...
	    std::cout << llama_tokenizer.tokenizer_decode(next_tokens);
