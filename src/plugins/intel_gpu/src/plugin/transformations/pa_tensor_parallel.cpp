...
PATensorParallelFusion::PATensorParallelFusion(size_t world_size, size_t world_rank) {
    ...
    ov::matcher_pass_callback callback = [=](ov::pass::pattern::Matcher& m) {
        ...
        auto split_fc = [&](std::shared_ptr<ov::Node>& org_fc,
            ...
            std::cout << "[fc]: " << org_fc->get_friendly_name() << std::endl;
            ...
            split_dim_range = reshaped_pshape.to_shape()[split_axis];
            {
                ...
                if (compressed_fc) {
                    // std::cout << "m_data: " << m_data->get_output_partial_shape(0) << std::endl;
                    // std::cout << "ranked_weight: " << ranked_weight->get_shape() << std::endl;
                    // std::cout << "m_bias: " << m_bias->get_shape() << std::endl;
                    // std::cout << "ranked_scale: " << ranked_scale->get_shape() << std::endl;
                    if (compressed_fc->inputs().size() > 4)
                ...
                // std::cout << "out shape: " << splitted_fc->get_output_partial_shape(0) << std::endl;
        ...
        auto new_add_node = [&](std::shared_ptr<ov::Node>& add_node, const std::vector<int64_t> qkv_parts) {
            ...
            std::cout << "[add]" << add_node->get_friendly_name() << ": in:" << add_node->get_input_partial_shape(0)
                      << ", old out: " << add_node->get_output_partial_shape(0)
                      << ", new out:" << new_add->get_output_partial_shape(0) << std::endl;
        ...
        if (m_pa) {
            ...
            std::cout << "half_head_num: " << half_head_num << ", head_size: " << head_size << ", q_size: " << q_size
                      << ", k_size: " << k_size << ", v_size: " << v_size << std::endl;
