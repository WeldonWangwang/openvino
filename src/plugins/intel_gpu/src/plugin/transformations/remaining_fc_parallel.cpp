    ...
    ov::matcher_pass_callback callback = [=](ov::pass::pattern::Matcher& m) {
        ...
        };
        {
            ...
            new_fc->get_rt_info().insert({"splitted", true});
            {
                ...
                for (auto u : new_fc->get_users()) {
                    for (size_t idx = 0; idx < u->inputs().size(); ++idx) {
                        if (u->get_input_node_shared_ptr(idx) == new_fc) {
                            org_users.insert({idx, u});
                        }
                    }
                }
                std::shared_ptr<ov::intel_gpu::op::SyncTensor> sync_node;
                sync_node = std::make_shared<ov::intel_gpu::op::SyncTensor>(new_fc, world_size, world_rank, splitted_context.second,
