...
void pa_lsc_u8(
    ...
    for(int kv_pos = 0; kv_pos < kv_stop; kv_pos += kv_step,slm_buff_id_read++) {
        ...
        // Reduces unnecessary online softmax iterations that accumulate error.
        if constexpr (use_causal_mask) {
            if (causal_left < 0) {
                causal_left -= kv_step;
                continue;
...
void pa_kernel_lsc_prefetch_f16(
    ...
    for(int kv_pos = 0; kv_pos < kv_stop; kv_pos += kv_step) {
        ...
        // Post-scale QK scores in fp32 (avoids (half)scale_factor truncation that
        // compounds over many online softmax iterations for long sequences)
        St = cm_mul<float>(St, (float)scale_factor);
        if constexpr (use_causal_mask) {
            // since kv_step == q_step == 16, causal_left is n*kv_step
            if (causal_left == 0) {
