...
void dequantize_iq3_s(const uint8_t* src, size_t total, std::vector<ov::float16>& out) {
    ...
    for (size_t base = 0; base < total; base += kSuper, src += kBytes) {
        ...
        for (int ib32 = 0; ib32 < 8; ib32 += 2) {
            ...
            for (int l = 0; l < 4; ++l) {
                const auto* grid1 = reinterpret_cast<const uint8_t*>(&iq3s_grid[qs[2 * l + 0] | ((qh[0] << (8 - 2 * l)) & 256)]);
                const auto* grid2 = reinterpret_cast<const uint8_t*>(&iq3s_grid[qs[2 * l + 1] | ((qh[0] << (7 - 2 * l)) & 256)]);
                for (int j = 0; j < 4; ++j) {
            ...
            signs += 4;
            for (int l = 0; l < 4; ++l) {
                const auto* grid1 = reinterpret_cast<const uint8_t*>(&iq3s_grid[qs[2 * l + 0] | ((qh[1] << (8 - 2 * l)) & 256)]);
                const auto* grid2 = reinterpret_cast<const uint8_t*>(&iq3s_grid[qs[2 * l + 1] | ((qh[1] << (7 - 2 * l)) & 256)]);
                for (int j = 0; j < 4; ++j) {
...
std::shared_ptr<ov::op::v0::Constant> dequantize_to_f16(const GGUFReader& reader, const std::string& name) {
    ...
    if (type == ov::element::f32) {
        ...
        OPENVINO_THROW("[GGUF Frontend] Embedding tensor '",
                       ...
                       type.get_type_name(),
                       "'. Supported embedding types: F32, F16, BF16, Q8_0, Q4_0, Q4_1, Q3_K, Q4_K, Q5_K, Q6_K, IQ2_XXS, IQ2_XS, IQ2_S, IQ3_XXS, IQ3_S, IQ4_XS.");
    }
